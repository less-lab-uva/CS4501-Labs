...
initPose_(7,0)
{
  ...
  if (!ros::param::get("/uav/flightgoggles_uav_dynamics/clockscale", clockScale)) {
    std::cout << "Using sim_time and did not get a clock scaling value. Defaulting to 1." << std::endl;
  }
  
  if (!ros::param::get("/uav/flightgoggles_uav_dynamics/position_noise_lvl", position_noise_lvl)) {
    std::cout << "Did not find value for position noise level. Defaulting to 0." << std::endl;
  }
  
  if (!ros::param::get("/uav/flightgoggles_uav_dynamics/velocity_noise_lvl", velocity_noise_lvl)) {
    std::cout << "Did not find value for velocity noise level. Defaulting to 0." << std::endl;
  }
  
  if (!ros::param::get("/uav/flightgoggles_uav_dynamics/attitude_noise_lvl", attitude_noise_lvl)) {
    std::cout << "Did not find value for attitude noise level. Defaulting to 0." << std::endl;
  }
...
//  initialPose.position.y = 2.0f;
  ...
  gpsPub_ = node_.advertise<geometry_msgs::PoseStamped>("/uav/sensors/gps", 96);
  gpsNoisyPub_ = node_.advertise<geometry_msgs::PoseStamped>("/uav/sensors/gps_noisy", 96);
  groundTruthPub_ = node_.advertise<geometry_msgs::PoseStamped>("/uav/sensors/ground_truth", 96);
  velPub_ = node_.advertise<geometry_msgs::TwistStamped>("/uav/sensors/velocity", 96);
  velNoisyPub_ = node_.advertise<geometry_msgs::TwistStamped>("/uav/sensors/velocity_noisy", 96);
  armPub_ = node_.advertise<std_msgs::Bool>("/uav/armed", 96);
...
/**
 * Returns a value randomly sampled from the described normal distribution
 */
double Uav_Dynamics::normal_distribution(double mean, double str_dev) {
//      return mean;
    double z = standardNormalDistribution_(randomNumberGenerator_);
    return z * str_dev + mean;
}
...
void Uav_Dynamics::publishState(void){
  ...
  // TODO add noise to the odom tf?
  geometry_msgs::TransformStamped imuLevelTransform;

  imuLevelTransform.header.stamp = currentTime_;
  imuLevelTransform.header.frame_id = "world";

  imuLevelTransform.transform.translation.x = position_[0];
  imuLevelTransform.transform.translation.y = position_[1];
  imuLevelTransform.transform.translation.z = position_[2];

  tf::Quaternion quat(attitude_[0], attitude_[1], attitude_[2], attitude_[3]);
  tf::Matrix3x3 mat(quat);
  double roll, pitch, yaw;
  mat.getRPY(roll, pitch, yaw, 1);
  mat.setRPY(0, 0, yaw); // only preserve the yaw
  mat.getRotation(quat);

  imuLevelTransform.transform.rotation.x = quat[0];
  imuLevelTransform.transform.rotation.y = quat[1];
  imuLevelTransform.transform.rotation.z = quat[2];
  imuLevelTransform.transform.rotation.w = quat[3];

  imuLevelTransform.child_frame_id = "uav/imu_level";

  tfPub_.sendTransform(imuLevelTransform);
  ...
  GPSReading.pose.position.x = normal_distribution(position_[0], position_noise_lvl);
  GPSReading.pose.position.y = normal_distribution(position_[1], position_noise_lvl);
  GPSReading.pose.position.z = normal_distribution(position_[2], position_noise_lvl);

  GPSReading.pose.orientation.x = normal_distribution(attitude_[0], attitude_noise_lvl);
  GPSReading.pose.orientation.y = normal_distribution(attitude_[1], attitude_noise_lvl);
  GPSReading.pose.orientation.z = normal_distribution(attitude_[2], attitude_noise_lvl);
  GPSReading.pose.orientation.w = normal_distribution(attitude_[3], attitude_noise_lvl);
  
  gpsNoisyPub_.publish(GPSReading);
  
  geometry_msgs::PoseStamped groundTruthReading;

  groundTruthReading.header.frame_id = "world";
  groundTruthReading.header.stamp = currentTime_;
  
  groundTruthReading.pose.position.x = position_[0];
  groundTruthReading.pose.position.y = position_[1];
  groundTruthReading.pose.position.z = position_[2];

  groundTruthReading.pose.orientation.x = attitude_[0];
  groundTruthReading.pose.orientation.y = attitude_[1];
  groundTruthReading.pose.orientation.z = attitude_[2];
  groundTruthReading.pose.orientation.w = attitude_[3];

  groundTruthPub_.publish(groundTruthReading);
  gpsPub_.publish(groundTruthReading);
  ...
  VelocityReading.twist.linear.x = normal_distribution(velocity_[0], velocity_noise_lvl);
  VelocityReading.twist.linear.y = normal_distribution(velocity_[1], velocity_noise_lvl);
  VelocityReading.twist.linear.z = normal_distribution(velocity_[2], velocity_noise_lvl);

  velNoisyPub_.publish(VelocityReading);

  geometry_msgs::TwistStamped GroundTruthVelocityReading;

  GroundTruthVelocityReading.header.frame_id = "world";
  GroundTruthVelocityReading.header.stamp = currentTime_;

  GroundTruthVelocityReading.twist.linear.x = velocity_[0];
  GroundTruthVelocityReading.twist.linear.y = velocity_[1];
  GroundTruthVelocityReading.twist.linear.z = velocity_[2];

  velPub_.publish(GroundTruthVelocityReading);
  // *
