...
for key in output_data:
    ...
    # Save the first 20% as test and 80% as train
    test_percentage = 0.2

    # Create a save index
    for index in range(len(shuffled_data)):
        ...
        # If this is the last image also save it as a single prediction image
        if index >= len(shuffled_data) - 1:
            save_name = "./testing_data/" + key + ".jpeg"
            cv2.imwrite(save_name, img)